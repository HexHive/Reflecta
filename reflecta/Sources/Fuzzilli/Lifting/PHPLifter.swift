        ...
        for instr in program.code {
            ...
            case .loadFloat(let op):
                ...
                if op.value.isNaN {
                    ...
                    expr = UnaryExpression.new("-INF")
                } else if op.value.isEqual(to: Double.infinity) {
                    expr = Identifier.new("INF")
                } else {
            ...
            case .loadBuiltin(let op):
                let bname = op.builtinName == "Reflecta" ? "$reflecta" : op.builtinName
                w.assign(Identifier.new(bname), to: instr.output)
            ...
            case .beginForLoop(let op):
                ...
                w.emit("for (\(I) = \(INITIAL); \(COND); \(EXPR)) {")
                w.enterNewBlock()
