...
public class RubyLifter: Lifter {
        ...
        for instr in program.code {
            ...
            case .loadFloat(let op):
                ...
                if op.value.isNaN {
                    ...
                    expr = UnaryExpression.new("-Float::INFINITY")
                } else if op.value.isEqual(to: Double.infinity) {
                    expr = Identifier.new("Float::INFINITY")
                } else {
            ...
            case .getProperty(let op):
                ...
                var prop = op.propertyName
                if !op.propertyName.allSatisfy({ch in ch.isNewline}) {
                    prop = "\(prop)"
                }
                let expr = MemberExpression.new() + obj + "::" + prop
                w.assign(expr, to: instr.output)
            ...
            //     let expr = UnaryExpression.new() + obj + ".delete(" + prop + ")"
            //     w.assign(expr, to: instr.output)
            ...
            case .construct:
                let f = w.retrieve(expressionFor: instr.input(0))
                let args = instr.variadicInputs.map({ w.retrieve(expressionFor: $0) })
                let EXPR = NewExpression.new() + f + ".new" + "(" + liftCallArguments(args) + ")"
            ...
            case .beginForLoop:
                ...
                let STOP = w.retrieve(expressionFor: instr.input(1))
                w.emit("for \(I) in \(INITIAL)..\(STOP)")
                w.enterNewBlock()
            ...
            default:
                fatalError("Unsupported instruction: \(instr)")
            }
        }
