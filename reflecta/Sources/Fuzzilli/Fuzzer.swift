...
public class Fuzzer {
    ...
    public init(
        ...
        environment: Environment, lifter: Lifter, corpus: Corpus, minimizer: Minimizer, queue: DispatchQueue? = nil, lang: String = "js"
    ) {
        let uniqueId = UUID()
        self.id = uniqueId
    ...
    func processMaybeInteresting(_ program: Program, havingAspects aspects: ProgramAspects, origin: ProgramOrigin) -> Bool {
        ...
        func finishProcessing(_ program: Program) {
            ...
            assert(!program.code.contains(where: { $0.op is BeginTry }))
    ...
    func processCrash(_ program: Program, withSignal termsig: Int, withStderr stderr: String, withStdout stdout: String, origin: ProgramOrigin, withExectime exectime: TimeInterval) {
        func processCommon(_ program: Program) {
            ...
            let execution = execute(program, withTimeout: self.config.timeout)
    ...
    private func makeComplexProgram() -> Program {
        ...
        let f = b.buildPlainFunction(with: .parameters(n: 2)) { params in
            let x = b.getElement(0, of: params[0])
            let y = b.getElement(1, of: params[0])
            let s = b.binary(x, y, with: .Add)
            let p = b.binary(s, params[1], with: .Mul)
            b.buildTryCatchFinally(tryBody: {
                b.throwException(b.loadNull())
            })
            b.doReturn(p)
        }

        b.buildForLoop(b.loadInt(0), .lessThan, b.loadInt(100), .Add, b.loadInt(1)) { i in
            let x = b.loadInt(42)
            let y = b.loadInt(43)
            let arg1 = b.createArray(with: [x, y])
            let arg2 = i
            b.doPrint(x)
    ...
    public func runStartupTests() {
        ...
        for _ in 0..<5 {
            let execution = execute(complexProgram, withTimeout: 1000)
            guard (execution.outcome == .succeeded) else {
                logger.info(lifter.lift(complexProgram))
                logger.error("Outcome: \(execution.outcome)")
                logger.error("STDOUT:\n\(execution.stdout)")
                logger.error("STDERR:\n\(execution.stderr)")
                logger.error("FUZZOUT:\n\(execution.fuzzout)")
                logger.fatal("Cannot execute complex program!")
            }
            print(execution.execTime)
        ...
        let p = b.finalize()
        let e = execute(p)
        let output = e.fuzzout.trimmingCharacters(in: .whitespacesAndNewlines)
