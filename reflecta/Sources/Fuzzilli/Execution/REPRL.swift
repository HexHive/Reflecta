...
public class REPRL: ComponentBase, ScriptRunner {
    ...
    public init(executable: String, processArguments: [String], processEnvironment: [String: String], maxExecsBeforeRespawn: Int, lang: String = "js") {
        /* Reflecta */
        self.lang = lang
        self.fuzzoutPath = "/tmp/\(getpid())"
        env.append("fuzzout=\(fuzzoutPath)")

        print("fuzzout=", fuzzoutPath)
    ...
    public func reset() {
        ...
        let _ = script.withCString {
            reprl_execute(reprlContext, $0, UInt64(script.count), 1000 * 1000, &execTime, 1)
        }
    }
    ...
    public func run(_ script: String, withTimeout timeout: UInt32) -> Execution {
        ...
        try? ("").write(toFile: fuzzoutPath, atomically: false, encoding: .utf8)
...
class REPRLExecution: Execution {
    ...
    var fuzzout: String {
        if !outputStreamsAreValid {
            self.reprl.reset()
            return ""
        }

        if self.reprl.lang == "js" {
            if cachedFuzzout == nil {
                cachedFuzzout = String(cString: reprl_fetch_fuzzout(reprl.reprlContext))
            }
            return cachedFuzzout!
        } else {
            // Read fuzzout as String from file /tmp/fuzzout
            if cachedFuzzout == nil {
                if let fuzzoutData = try? Data(contentsOf: URL(fileURLWithPath: reprl.fuzzoutPath)) {
                    cachedFuzzout = String(data: fuzzoutData, encoding: .utf8) ?? ""
                    return cachedFuzzout!
                } else {
                    reprl.logger.warning("Failed to read fuzzout file")
                    reprl.logger.warning("Outcome: \(outcome)")
                    reprl.logger.warning("Stdout:\n\(stdout)")
                    reprl.logger.warning("Stderr:\n\(stderr)")
                    return ""
                }
            } else {
                return cachedFuzzout ?? ""
