...
/* Reflecta */
func makeReflecta(for profile: Profile, with configuration: Configuration) -> Fuzzer {
    // A script runner to execute JavaScript code in an instrumented JS engine.
    let runner = REPRL(executable: jsShellPath, processArguments: profile.getProcessArguments(randomizingArguments), processEnvironment: profile.processEnv, maxExecsBeforeRespawn: profile.maxExecsBeforeRespawn, lang: profile.lang)

    // let engine: FuzzEngine = MutationEngine(numConsecutiveMutations: 6)
    let engine: FuzzEngine = ReflectionEngine() // MutationEngine(numConsecutiveMutations: 6)
    ...
    // The environment containing available builtins, property names, and method names.
    // let environment = JavaScriptEnvironment(additionalBuiltins: profile.additionalBuiltins, additionalObjectGroups: [])
    let environment = ReflectionEnvironment(additionalBuiltins: [:])
    ...
    switch profile.lang {
        case "js":
            lifter = JavaScriptLifter(prefix: profile.codePrefix,
                                        suffix: profile.codeSuffix,
                                        ecmaVersion: profile.ecmaVersion)
        case "ruby":
            lifter = RubyLifter(prefix: profile.codePrefix)

        case "python":
            lifter = PythonLifter(prefix: profile.codePrefix)

        case "php":
            lifter = PHPLifter(prefix: profile.codePrefix)

        default:
            fatalError("Unsupported lang: \(profile.lang)")
    ...
    // Corpus managing interesting programs that have been found during fuzzing.
    let corpus = BasicCorpus(minSize: minCorpusSize, maxSize: maxCorpusSize, minMutationsPerSample: minMutationsPerSample)

    // Minimizer to minimize crashes and interesting programs.
    let minimizer = Minimizer()

    // Filter out generators that generate unsupported instructions
    let allowedGenerators = [
        "IntegerGenerator",
        "BigIntGenerator",
        "RegExpGenerator",
        "FloatGenerator",
    ...
    let filteredGenerators = codeGenerators.filter { allowedGenerators.contains($0.name) }
    ...
    let mutators = WeightedList<Mutator>([
        ...
        /* Reflecta */
        (ReflectionMutator(),             100),

        /* JS Specific Mutators */
        // (InputMutator(isTypeAware: true),   1),
        // (ExplorationMutator(),              3),
        // (ProbingMutator(),                  2),
        // (JITStressMutator(),                1),
    ...
    return Fuzzer(configuration: config,
                  ...
                  engine: engine,
                  mutators: mutators,
                  codeGenerators: filteredGenerators,
                  programTemplates: programTemplates,
                  evaluator: evaluator,
                  environment: environment,
                  lifter: lifter,
...
let fuzzer = makeReflecta(for: profile, with: config)
